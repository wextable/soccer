//
//  LayoutConstraining+EdgesInside.swift
//  GlassUI
//
...
public extension LayoutAnchorable {
    ...
    /// Creates constraints that pin some (or all) of the edges to another view.
    ///
    ...
    /// - Parameter insets: Edge insets. Edges that are constrained (by passing `true`) are constrained relative to the
    ///                     insets. Default values are `0`.
    ...
    func constraints(pinningInside other: LayoutAnchorable,
                     edges: LayoutConstrainingEdges = .all,
                     insets: LayoutConstrainingInsets = 0,
                     priority: UILayoutPriority = .required) -> LayoutConstraining {
        [
            edges.contains(.top) ? topAnchor.constraint(greaterThanOrEqualTo: other.topAnchor,
                                                        constant: insets.top) : nil,
            edges.contains(.leading) ? leadingAnchor.constraint(greaterThanOrEqualTo: other.leadingAnchor,
                                                                constant: insets.leading) : nil,
            edges.contains(.bottom) ? bottomAnchor.constraint(lessThanOrEqualTo: other.bottomAnchor,
                                                              constant: -insets.bottom) : nil,
            edges.contains(.trailing) ? trailingAnchor.constraint(lessThanOrEqualTo: other.trailingAnchor,
