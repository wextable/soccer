...
/// A base class for `UICollectionViewCell`s in Glass.
///
/// The main feature of `BaseCollectionViewCell` is conformance to `ViewConstructable`.
///
...
///
/// A secondary feature is that subclasses don't need to implement `init(coder:)`. `BaseCollectionViewCell` marks that
/// initializer as unavailable.
open class BaseCollectionViewCell: UICollectionViewCell, ViewConstructable {
