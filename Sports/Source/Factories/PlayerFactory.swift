...
class PlayerFactory {
    ...
    static func makePlayer(position: Position = .forward) -> Player {
        ...
        let xpLevel = makeXpLevel(from: ratings)
        return Player(id: UUID().uuidString,
                      firstName: NameFactory.makeName(),
                      lastName: NameFactory.makeName(),
...
extension PlayerFactory {
    ...
    private static func makeCondition() -> Int {
...
extension PlayerFactory {
    ...
    private static func makeKeeperRating() -> Player.Ratings {
        ...
        let defending = Int.random(in: 60 ..< 100)
        let goalkeeping = Int.random(in: 60 ..< 100)

        return .init(position: .keeper,
                     speed: speed,
                     shooting: shooting,
                     passing: passing,
                     dribbling: dribbling,
    ...
    private static func makeMidfielderRating() -> Player.Ratings {
        ...
        let passing = Int.random(in: 60 ..< 100)
        let dribbling = Int.random(in: 55 ..< 95)
        let defending = Int.random(in: 50 ..< 95)
        let goalkeeping = Int.random(in: 20 ..< 40)

        return .init(position: .midfielder,
                     speed: speed,
                     shooting: shooting,
                     passing: passing,
                     dribbling: dribbling,
    ...
    private static func makeForwardRating() -> Player.Ratings {
        ...
        let shooting = Int.random(in: 55 ..< 100)
...
extension PlayerFactory {
    private static func makePotential(from ratings: Player.Ratings,
        ...
        let avgGrowth = potentialGrowth(currentRating: ratings.overall)
...
extension PlayerFactory {
    private static func makeXpLevel(from ratings: Player.Ratings) -> Int {
        switch ratings.overall {
